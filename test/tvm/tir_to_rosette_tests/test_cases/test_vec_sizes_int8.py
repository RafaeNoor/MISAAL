...
def test_vec_add_int8():
    vec_sizes = [2,4,8,16,32,64]
    for vs in vec_sizes:
        @I.ir_module
        class Module:
            @T.prim_func(private=True)
            def vec_add(x: T.Buffer((T.int64(1), T.int64(128)), "int8"), y: T.Buffer((T.int64(1), T.int64(128)), "int8"), T_add_intermediate_1: T.Buffer((T.int64(1), T.int64(128)), "int8")):
            ...
            @R.function
            def forward(x: R.Tensor((1, 128), dtype="int8"), y: R.Tensor((1, 128), dtype="int8")) -> R.Tensor((1, 128), dtype="int8"):
                R.func_attr({"num_input": 2})
                cls = Module
                with R.dataflow():
                    gv = R.call_tir(cls.vec_add, (x, y), out_sinfo=R.Tensor((1, 128), dtype="int8"))
                    R.output(gv)
                return gv
        mod = Module
        answer_str = f"""

...
def CompileSelect(Select, Context : DRAMRoseContext):
  ...
  # Set the output type of then and otherwise as 8-bits

  print("SELECT",Select)
  if isinstance(Select.then, Number) and  isinstance(Select.otherwise, Number):
      NewThen  = RoseConstant.create(Select.then.val, RoseBitVectorType.create(1))
      NewElse = RoseConstant.create(Select.otherwise.val, RoseBitVectorType.create(1))
      print(NewThen, NewElse)
      Then = NewThen
      Otherwise = NewElse
  else:
  ...
  assert isinstance(Operation.getType(), RoseBitVectorType), "EXPECTED SELECT TO PRODUCE BV"
...
def CompileBinaryExpr(BinaryExpr, Context : DRAMRoseContext):
  ...
  if ExtendOperandSize == True:
    ...
    # Temp: return truncated sizes
    TruncateFname = 'Truncate'+str(OperandBitwidth // 2)
    NewOp = Builtins[TruncateFname](Context.genName(), [Operation], Context)
    # Add the operation to the IR
    Context.addAbstractionToIR(NewOp)
    Context.addCompiledAbstraction(BinaryExpr.id, NewOp)
    print("NewOp", NewOp)
    return NewOp
  return Operation
...
def HandleToMul():
  def LamdaImplFunc(Name : str, Operand1 : RoseValue, Operand2 : RoseValue, \
    ...
    if isinstance(Operand1.getType(), RoseBitVectorType) \
    and isinstance(Operand2.getType(), RoseBitVectorType):
      print("Handle to Mul Operands", Operands)

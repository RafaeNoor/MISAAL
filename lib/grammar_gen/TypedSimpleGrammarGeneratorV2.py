...
class TypedSimpleGrammarGeneratorV2:
    ...
    def emit_typed_grammar_clause(self, dsl_inst, ctx , vars_name = "vars",
                             depth_name = "k", layer_output_len = 256, layer_output_prec = 8,
                            last_clause = False):
        ...
        layer_name = self.get_layer_name(layer_output_len, layer_output_prec)
        ...
        for ga in grammar_args:
            ...
            if ga.is_hole:
                ...
                grammar_name = self.get_layer_name(ga.size, ctx.in_precision)

                if layer_name in self.reachable_sizes:
                    self.reachable_sizes.append(grammar_name)

                self.init_typed_context(ga.size, ctx.in_precision)
    ...
    def emit_typed_grammar_layer(self, layer_name = "grammar_layer", layer_dsl_insts = [], layer_dsl_args_list = [],
                                 ...
                                 lit_holes = [], input_sizes = [], layer_output_prec = 8):

        clauses = []
        layer_output_len = int(layer_output_len)
        layer_output_prec = int(layer_output_prec)

        if "start" in layer_name:
            self.reachable_sizes.append(layer_name)
        ...
        for idx in range(0, len(layer_dsl_insts)):
            clauses.append(
                self.emit_typed_grammar_clause(layer_dsl_insts[idx],
                                               ...
                                               layer_output_prec = layer_output_prec,
    ...
    def get_vector_type_name(self, output_size, output_precision):

        return str(output_size)+"_"+str(output_precision)

    def init_typed_context(self, output_size, output_precision):
        name = self.get_vector_type_name(output_size, output_precision)
        if name not in self.typed_contexts:
            self.typed_contexts[name] = { "ops": [], "ctx": [] , "name": self.get_layer_name(output_size, output_precision), "lits" : [], "regs":  [] }
    ...
    def seperate_contexts_by_type(self, operation_dsl_insts,
        ...
        for idx in range(0, len(operation_dsl_insts)):
            ...
            self.init_typed_context(output_size, output_prec)


            name = self.get_vector_type_name(output_size, output_prec)
            self.typed_contexts[name]["ops"].append(op)
            self.typed_contexts[name]["ctx"].append(ctx)


        for lit in lit_holes:
            self.init_typed_context(lit[0], lit[0])

        for idx, inp in enumerate(input_sizes):
            self.init_typed_context(inp, self.input_precs[idx])


    def get_layer_name(self, bv_type, bv_prec):
        bv_type = int(bv_type)
        bv_prec = int(bv_prec)

        if bv_type == self.return_type and bv_prec == bv_type:
            return "expr"+ "_start"
        else:
            return "expr" + "-bv"+str(bv_type)+"-e"+str(bv_prec)
    ...
    def emit_grammar(self,
        ...
        for bv_name, grammar_context in list(self.typed_contexts.items()):
            bv_sizes = bv_name.split("_")[0]
            bv_precs = bv_name.split("_")[1]

            grammar_name = self.get_layer_name(bv_sizes, bv_precs)
            #print("Grammar NAME:", grammar_name)

            grammar = self.emit_typed_grammar_layer(layer_name = self.get_layer_name(bv_sizes, bv_precs),
                                                    layer_dsl_insts = grammar_context['ops'],
                                                    layer_dsl_args_list = grammar_context['ctx'],
                                                    layer_output_len = bv_sizes,
                                                    lit_holes = lit_holes,
                                                    input_sizes = input_sizes,
                                                    layer_output_prec = bv_precs
                                                    )

            size_to_expr_map[bv_name] = grammar
            size_to_expr_map[grammar_name] = grammar
        ...
        for i in range(0,5):
            for bv_name, grammar_context in self.typed_contexts.items():

                bv_sizes = bv_name.split("_")[0]
                bv_precs = bv_name.split("_")[1]
                self.emit_typed_grammar_layer(layer_name = self.get_layer_name(bv_sizes, bv_precs),
                                                        layer_dsl_insts = grammar_context['ops'],
                                                        layer_dsl_args_list = grammar_context['ctx'],
                                                        layer_output_len = bv_sizes,
                                                        lit_holes = lit_holes,
        ...
        self.reachable_sizes = list(set(self.reachable_sizes))

        grammars = [size_to_expr_map[bv_sizes] for bv_sizes in size_to_expr_map if bv_sizes in self.reachable_sizes]

        #print(size_to_expr_map.keys())
        #print("Reachable layers:", self.reachable_sizes)
        ...
        grammar_depth_fn = "(define ({} k) ({} 'a #:depth k #:start {}))\n".format(operation_layer_name, simple_grammar_name,
                                                                                   self.get_layer_name(return_type, return_type))

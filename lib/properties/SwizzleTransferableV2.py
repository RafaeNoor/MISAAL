...
class SwizzleTransferableV2(EqClassEqualDepthV4):
    ...
    def get_relavent_swizzle_dsl_subset(self, dsl_inst):
        return [sw for sw in self.swizzle_dsl_list if sw.name != "hvx_swizzle_49"]

    def get_relavent_output_dsl_subset(self, dsl_inst):
        return [dsl_inst] + self.get_relavent_swizzle_dsl_subset(dsl_inst)
                    ...
                    for src_expr in src_expressions:
                        ...
                        for target_count ,target_expr in enumerate(target_expressions):
                            ...
                            if get_expr_depth(target_expr) == output_depth or isinstance(target_expr, Reg):
                                ...
                                # If both expressions don't contain swizzles then don't process
                                if self.count_contexts(src_expr, "swizzle") + self.count_contexts(target_expr, "swizzle") == 0:
                                    continue

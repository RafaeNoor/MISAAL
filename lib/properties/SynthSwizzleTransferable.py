...
class SynthSwizzleTransferable(SwizzleTransferable):
    ...
    def __init__(self, dsl_list = [], synth_desc = None, swizzles = []):
        super().__init__(name = "SynthSwizzleTransfer", dsl_list = dsl_list, synth_desc = synth_desc, swizzles = swizzles)
    ...
    def property_holds_on_candidate(self, candidate):
        ...
        combined_synth_desc = create_synth_desc("swizzle_ctx",True, self.synth_desc.target_sizes, sema_path ,dict_name)



        (is_simplified, simplified_expr) =  translate_expression(synth_spec, self.synth_desc, combined_synth_desc, input_sizes, input_precs, src_language_dsl = [dsl_inst, swizzle_inst], target_language_dsl = combined_list)

...
class Associative(Property):
    ...
    def get_property_desc(self):
        return "Check Associative for DSLInstructions"
    ...
    def generate_candidates(self):
        ...
        for dsl_inst in self.dsl_list:
            associatable_args = self.get_associatable_args_candidates_instruction(dsl_inst)
            for pairs in associatable_args:
                tuples.append((dsl_inst, pairs))

        return tuples

    def get_associatable_args_candidates_instruction(self, dsl_inst):
        """Identify the associatable arg candidates which can be applied on the overall equivalence class. We currently
    ...
    def get_associatable_args_candidates_context(self, ctx):
        """Returns a list of 2-tupples for arguments for a given context
        which may be associatable. The criterion for this requires that the operands have the
        ...
        for size_key in arg_sizes_map:
            ...
            for i in range(0, len(arg_indices)):
                for j in range(i+1 , len(arg_indices)):
                    associatable_args.append((arg_indices[i], arg_indices[j]))


        return associatable_args

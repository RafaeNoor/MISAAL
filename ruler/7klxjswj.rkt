...
(define (double_target_:bind-expr prog env)
 (destruct prog
	...
	[(idx-j id) (idx-j id)]
	[(reg id) (vector-ref-bv env id)]
	[(buffer-index id type size) (vector-ref env id)]
	[(lit v) (lit v)]
...
(define (invoke-spec src-expr  env)
 (cond [(not (concrete? src-expr)) (assert (and (contains_reg src-expr 0)))])
...
(cond [satisfiable? (write-str-to-file (~v mat-src) "unap70yi.src.log.rkt")
(write-str-to-file (~v mat-dst) "unap70yi.dst.log.rkt")
(exit 0)] [else (exit 1)])

...
class Convolution : public Generator<Convolution> {
    public:
        ...
        void generate() {
            ...
            // Specialize the update schedule for filter_x == 3
            Output.update()
                .specialize(Filter.dim(0).extent() == 3)
                .specialize(Filter.dim(1).extent() == 3)
                .specialize(Filter.dim(2).extent() == 64)
                .specialize(IMG.dim(0).extent() == 256)
                .specialize(IMG.dim(1).extent() == 256)
                .specialize(IMG.dim(2).extent() == 3)
                .specialize(Output.dim(0).extent() == 256)
                .specialize(Output.dim(1).extent() == 256)
                .specialize(Output.dim(2).extent() == 64)
                .fuse(x, y, x)
                .fuse(x, c_out, x)

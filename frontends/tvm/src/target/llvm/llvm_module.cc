...
void LLVMModuleNode::Init(const IRModule& mod, const Target& target) {
  ...
  Optional<Bool> should_print = pass_ctx->GetConfig("codegen_debug", Bool(false));
  if (should_print.value() == true){
    std::cerr << std::endl << std::endl << "===LLVMModuleNode::Init IRModule DUMP===" << std::endl << std::endl;
    tvm::Dump(mod);
    std::cerr << std::endl << std::endl << "===LLVMModuleNode::Init IRModule END DUMP===" << std::endl << std::endl;
  }
  ...
  std::unique_ptr<CodeGenLLVM> cg;
  Optional<Bool> use_misaal = pass_ctx->GetConfig("misaal", Bool(false));
  if (use_misaal.value() == true){
    const PackedFunc* f = runtime::Registry::Get("tvm.codegen.llvm.misaal");
    void* handle = (*f)();
    cg = std::unique_ptr<CodeGenLLVM>(static_cast<CodeGenLLVM*>(handle));
  } else {
    cg = CodeGenLLVM::Create(llvm_target.get());
  }
...
TVM_REGISTER_GLOBAL("callhello").set_body([](TVMArgs args, TVMRetValue* rv){
  PackedFunc f = args[0]; 
  f("hello world");
});
...
// Don't run the misaal compiler i.e. link an existing ll file
TVM_REGISTER_PASS_CONFIG_OPTION("disable_misaal_compile", Bool);

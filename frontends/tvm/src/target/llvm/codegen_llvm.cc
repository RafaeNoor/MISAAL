...
std::unique_ptr<llvm::Module> CodeGenLLVM::Finish() {
  ...
  if (should_print.value() == true){
    std::cerr << std::endl << std::endl << "===CodeGenLLVM::Finish pre optimization llvm::module DUMP===" << std::endl << std::endl;
    module_->print(llvm::errs(), nullptr);
    std::cerr << std::endl << std::endl << "===CodeGenLLVM::Finish pre optimization llvm::module END DUMP===" << std::endl << std::endl;
  }
  this->Verify();
  this->Optimize();
  this->Verify();
  if (should_print.value() == true){
    std::cerr << std::endl << std::endl << "===CodeGenLLVM::Finish llvm::module DUMP===" << std::endl << std::endl;
    module_->print(llvm::errs(), nullptr);
    std::cerr << std::endl << std::endl << "===CodeGenLLVM::Finish llvm::module END DUMP===" << std::endl << std::endl;
  }
...
llvm::Value* CodeGenLLVM::VisitExpr_(const BufferLoadNode* op) {
  ...
  auto make_load = [this, &loads, &value_dtype](TypedPointer buffer_ptr, int /* subelement_i */,
                                  llvm::Value* predicate, int alignment, bool is_volatile) {
